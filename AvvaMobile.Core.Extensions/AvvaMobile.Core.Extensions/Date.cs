...
namespace AvvaMobile.Core.Extensions
{
    public static class Date
    {
        ...
        public static (DateTime, DateTime) DataTableDateTime(this string date)
        {
            ...
            if (dateRange.Length == 2)
            {
                DateTime FilterStartDate =
                    DateTime.ParseExact(dateRange[0].Trim(), "MM/dd/yyyy", CultureInfo.InvariantCulture);
                DateTime FilterEndDate =
                    DateTime.ParseExact(dateRange[1].Trim(), "MM/dd/yyyy", CultureInfo.InvariantCulture);
        ...
        public static DeserializedDateTime ToDeserializedDateTime(this DateTime dateTime, string dayFormat = "ddd",
            string monthFormat = "MMM", bool isMonthUpper = true, bool isDayUpper = true)
        {
            var deserializedDateTime = new DeserializedDateTime
            {
                Day = dateTime.Day.ToString("D2"),
                DayName = dateTime.ToString(dayFormat),
                Month = dateTime.Month.ToString("D2"),
                MonthName = dateTime.ToString(monthFormat),
